        ...
        public Vm_rfm create(int conta_id, Vm_rfm_filtro filtro)
        {
            ...
            try
            {
                ...
                if(filtro.vm_rfm_filtros_visao == "caixa")
                {
                    ...
                    //dr_3_c.CommandText = "SELECT cg.categoria_id, cg.categoria_classificacao, cg.categoria_nome,(COALESCE((SELECT sum(b.oppb_valor + b.oppb_juros + b.oppb_multa - b.oppb_desconto) as 'caixa' from op_parcelas_baixa as b LEFT JOIN op_parcelas as p on p.op_parcela_id = b.oppb_op_parcela_id LEFT JOIN operacao as op on op.op_id = p.op_parcela_op_id LEFT JOIN categoria as c on c.categoria_id = op.op_categoria_id WHERE op.op_conta_id = @conta_id and b.oppb_data BETWEEN @data_inicial and @data_final and c.categoria_classificacao like concat(cg.categoria_classificacao,'%')),0)+COALESCE((SELECT sum(ccm.ccm_valor) from conta_corrente_mov as ccm LEFT JOIN categoria as c_ on c_.categoria_id = ccm.ccm_contra_partida_id WHERE ccm.ccm_conta_id = @conta_id and ccm.ccm_data BETWEEN @data_inicial and @data_final and ccm.ccm_origem = 'CCM' and c_.categoria_classificacao like concat(cg.categoria_classificacao,'%')),0)+COALESCE(round((SELECT sum(p.op_parcela_valor / (SELECT sum(p.op_parcela_valor) from op_parcelas as p LEFT JOIN operacao as op on op.op_id = p.op_parcela_op_id LEFT JOIN categoria as c on c.categoria_id = op.op_categoria_id LEFT JOIN forma_pagamento as fp on fp.fp_id = p.op_parcela_fp_id WHERE fp.fp_conta_id = @conta_id and fp.fp_meio_pgto_nfe = '03' and p.op_parcela_vencimento BETWEEN @data_inicial and @data_final)) * (SELECT sum(m.mcc_valor) from movimentos_cartao_credito as m LEFT JOIN fatura_cartao_credito as f on f.fcc_id = m.mcc_fcc_id WHERE f.fcc_conta_id = @conta_id and m.mcc_movimento = 'C' and m.mcc_data BETWEEN @data_inicial and @data_final) from op_parcelas as p LEFT JOIN operacao as op on op.op_id = p.op_parcela_op_id LEFT JOIN categoria as c on c.categoria_id = op.op_categoria_id LEFT JOIN forma_pagamento as fp on fp.fp_id = p.op_parcela_fp_id WHERE fp.fp_conta_id = @conta_id and fp.fp_meio_pgto_nfe = '03' and p.op_parcela_vencimento BETWEEN @data_inicial and @data_final and c.categoria_classificacao like concat(cg.categoria_classificacao,'%')),2),0)) as 'valor' from categoria as cg WHERE cg.categoria_conta_id = @conta_id;";
                }
                else
                {
